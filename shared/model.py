...
class Breed(BaseModel):
    breed_id = CharField(primary_key=True, max_length=50)
    origin_id = ForeignKeyField(Origin, index=True, backref='origins')
...
class Image_Category(BaseModel):
...
class Image_Breed(BaseModel):
    id = CharField(primary_key=True, max_length=50)
    breed_id = ForeignKeyField(Breed, index=True, on_delete='CASCADE', backref='images')
    url = CharField(max_length=400)
    class Meta:
        db_table = 'images_breed'

def init_database():
    _CLASSES = [Breed, Image_Category, Image_Breed, Category, Origin, Temperament, BreedTemperament]
    try:
